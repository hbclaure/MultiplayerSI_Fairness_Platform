...
#     1: Training,
#     2: EqualSupport,
#     3: UnfairSupportHumanEarly,
#     4: UnfairSupportHumanLate,
#     5: EqualSupport,
#     6: Apology, 
#     7: Apology
# }
...
class ControlHandler(tornado.websocket.WebSocketHandler):
    ...
    def set_agents(self, game_mode):
        ...
        if game_mode == "A":
            ...
            print('in B condition')
            self.agents = {
                0: NaoTrain,
                1: Training,
                2: EqualSupport,
                3: UnfairSupportShutterEarly,
                4: UnfairSupportShutterLate,
                5: EqualSupport,
                6: Apology, 
                7: Apology
            }
            ...
            self.agents = {
                ...
                1: Training,
                2: EqualSupport,
                3: UnfairSupportHumanLate,
                4: UnfairSupportHumanEarly,
                5: EqualSupport,
                6: Apology, 
                7: Apology
            }
            ...
            raise ValueError("Invalid game mode: {}".format(game_mode))
    ...
    def on_message(self, msg):
        if not self.mode and self.mode != 0:
            try:
                ...
                self.set_agents(str(state['game_condition'])) # Set the agent based on condition
                self.current_agent = self.agents[self.mode]()
...
class ImageHandler(tornado.websocket.WebSocketHandler):
    ...
    def on_close(self):
        ...
        if self.mode> 0:
            #Ignore recording the Nao practice round 

            #On close is called twice. Using counter to only run video creation once
            print(ImageHandler.close_count)
            ImageHandler.close_count += 1

            #Create Video
            if ImageHandler.close_count == 2:
                print('creating video')
                if self.folder:
                    image_folder = f"participant_data/P{self.player_id}/{self.dirname}/recorded_frames/{self.folder}/gamescreen"
                ...
                try:
                    folder = f"participant_videos/P{self.player_id}"
                    video_name = f"{folder}/P{self.player_id}_m{str(self.mode)}_g{str(self.game_num)}.mp4"
                    if not os.path.exists(os.path.dirname(video_name)):
                        os.makedirs(os.path.dirname(video_name))
                    

                    clip = ImageSequenceClip(image_files, fps=13.5)
                    clip.write_videofile(video_name)
                except Exception as e:
                    print(f"Error creating video: {e}")

                print('done making video')
                #reset counter because it stores counter in memory 
                ImageHandler.close_count =0
    ...
    def on_message(self, msg):
        if not self.player_id:
            try:
                ...
                self.game_num = str(gn)
                self.folder = f"m{str(self.mode)}_g{str(self.game_num)}_t{str(self.time_label)}"
