...
function update1 ()
{
    ...
    if (Date.now() - ai_ship.sprite.props.last_shot_time >= max_ai_frequency) {
        console.log('here',max_ai_frequency);
        ai_shoots = true;
    }
    ...
    if (players[0].sprite.props.dead | ai_ship.sprite.props.dead) {
    ...
    if (this.input.keyboard.checkDown(cursors.up, 0) && feedback_enabled) {
        if(frame_number >= last_msg_frame + frames_per_message) {
            ...
            players[0].sprite.props.message.setText("Good job");
        ...
        if (frame_number >= last_msg_frame + frames_per_message) {
            ...
            players[0].sprite.props.message.setText("Bad job");
            players[0].sprite.props.message.visible = true;
    ...
    if (ai_ready == false) {
        ...
        // update player
        players[0].update(player_left, player_right, player_shoots);
    ...
    var log_frame = {
        ...
        player_position: players[0].sprite.x,               //!< Player's position
        player_lives: players[0].sprite.props.lives,        //!< Player's lives
        player_score: players[0].sprite.props.score,        //!< Player's score
        ...
        player_last_shot_time: players[0].sprite.props.last_shot_time, // frame when player last shot
        ai_last_shot_time: ai_ship.sprite.props.last_shot_time,         // frame when ai last shot
        player_last_shot_frame: players[0].sprite.props.last_shot_frame, // frame when player last shot
