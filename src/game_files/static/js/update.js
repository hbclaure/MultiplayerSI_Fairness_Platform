...
function update ()
{
    ...
    if (gamepad1 == true) {
        ...
        if (this.input.keyboard.checkDown(space_key, 0)) {
            ...
            if (time_since_last_shot >= max_player_frequency) 
            {   
                // console.log('time since last shot', time_since_last_shot)
                // console.log('max player freq',max_player_frequency )
    ...
    if (Date.now() - ai_ship.sprite.props.last_shot_time >= max_ai_frequency) {
        //console.log('max frequency for nao', max_ai_frequency)
        ai_shoots = true;
    }
    ...
    var enemies_right_shot = []
    for (var i = 0; i < enemies_right_sprites.length; i++) {
        var current_enemy = enemies_right_sprites[i];
        enemies_right_shot.push(current_enemy.is_hit);
    }
    ...
    var log_frame = {
        ...
        enemies_right_shot: enemies_right_shot,
    ...
    if (player_over && ai_over) {
        ...
        game_log = {player_id: player_id, date: date, mode: mode, events: events, frames_not_sent: frames_not_sent};
